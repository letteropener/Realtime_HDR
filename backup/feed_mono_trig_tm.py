...
def run_single_camera(cam,to_proc,from_proc,TM_to_proc,TM_from_proc):
    """
        ...
        while True:
            ...
            if index==0:
                ...
                if cv2.waitKey(5) & 0xff==ord('q'):
                    break
                HDR_FRAME=from_proc.get()
                TM_to_proc.put(HDR_FRAME)
                HDR_FRAME = TM_from_proc.get()
            index+=1
...
def TM_proc(TM_in_queue,TM_out_queue):
    while True:
        try:
            v=TM_in_queue.get() #get
            if v is None:
                break
            TM_out_queue.put(ldr_tonemap_L_image(v/255,5,50))
        except queue.Empty:
            continue
        except Exception as e:
            raise e
if __name__ == "__main__":
    ...
    p2 = Process(target=TM_proc, args=(TM_to_proc,TM_from_proc))
    p.start()
    p2.start()

    cam = cam_list.GetByIndex(0)
    run_single_camera(cam,to_proc,from_proc,TM_to_proc,TM_from_proc)
    #-----------------------------------------------
    ...
    while from_proc.empty() is False:
        ...
        try:
            ...
            continue
    TM_to_proc.put(None)
    while TM_from_proc.empty() is False:
        print("in loop")
        try:
            print(TM_from_proc.get())
        except:
            continue
    p.join()
    p2.join()
