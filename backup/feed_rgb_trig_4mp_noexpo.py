...
from run_CCRF_mono import ldr_tonemap_rgb_image
    ...
    try:
        ...
        while True:
            ...
            if lv3_from_proc.empty() is False:
                HDR_FRAME = lv3_from_proc.get()
                #HDR_FRAME = ldr_tonemap_rgb_image(HDR_FRAME/255,5,50)
    ...
    while True:
        try:
            ...
            if v is None:
                break
            if v[1] == 1:  #v[1] == f2q
                out_frame[:,:,0] = parallel_HDR_B(v[0][:,:,0],frame1[:,:,0])
                out_frame[:,:,1] = parallel_HDR_G(v[0][:,:,1],frame1[:,:,1])
                out_frame[:,:,2] = parallel_HDR_R(v[0][:,:,2],frame1[:,:,2])
                lv2_in_queue.put((out_frame,0)) # frame1 + frame2 --> calc1
                frame2 = v[0]
            elif v[1] ==2: #v[1] == f4q
                out_frame[:,:,0] = parallel_HDR_B(v[0][:,:,0],frame2[:,:,0])
                out_frame[:,:,1] = parallel_HDR_G(v[0][:,:,1],frame2[:,:,1])
                out_frame[:,:,2] = parallel_HDR_R(v[0][:,:,2],frame2[:,:,2])
                lv2_in_queue.put((out_frame,1)) # frame2 + frame3 --> calc2
                #calc4 = parallel_CCRF(calc2[0],calc1[0])     # calc1 + calc2 --> calc4
                frame3 = v[0]
            elif v[1] ==3: #v[1] == f8q
                out_frame[:,:,0] = parallel_HDR_B(v[0][:,:,0],frame3[:,:,0])
                out_frame[:,:,1] = parallel_HDR_G(v[0][:,:,1],frame3[:,:,1])
                out_frame[:,:,2] = parallel_HDR_R(v[0][:,:,2],frame3[:,:,2])
    ...
    while True:
        try:
            ...
            if v is None:
                break
            if v[1] == 1:
                out_frame[:,:,0] = parallel_HDR_B(v[0][:,:,0],calc1[:,:,0])
                out_frame[:,:,1] = parallel_HDR_G(v[0][:,:,1],calc1[:,:,1])
                out_frame[:,:,2] = parallel_HDR_R(v[0][:,:,2],calc1[:,:,2])
                lv3_in_queue.put((out_frame,0)) # calc2 + calc1 --> calc4
                calc2 = v[0]
            elif v[1] == 2:
                out_frame[:,:,0] = parallel_HDR_B(v[0][:,:,0],calc2[:,:,0])
                out_frame[:,:,1] = parallel_HDR_G(v[0][:,:,1],calc2[:,:,1])
                out_frame[:,:,2] = parallel_HDR_R(v[0][:,:,2],calc2[:,:,2])
    ...
    while True:
        ...
        try:
            ...
            if v[1] == 1:
                ...
                HDR_FRAME[:,:,0] = parallel_HDR_B(v[0][:,:,0],calc4[:,:,0])
                HDR_FRAME[:,:,1] = parallel_HDR_G(v[0][:,:,1],calc4[:,:,1])
                HDR_FRAME[:,:,2] = parallel_HDR_R(v[0][:,:,2],calc4[:,:,2])
