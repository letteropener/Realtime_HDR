...
def run_single_camera(cam,to_proc,sharedK,sharedBaseExposure):
    ...
    try:
        ...
        while True:
            ...
            exposures=[(sharedK.value**i)*sharedBaseExposure.value for i in range(0,4)]
            #####black magic
...
def HDR_proc(in_queue, sharedK, sharedBaseExposure):
    ...
    debug_flag = True
    max_flag = False
    CCRF_mode_flag = True #True == 6 calculations False == 3 calculations
    canvas=np.zeros((FRAME_HEIGHT*2,FRAME_WIDTH*2,3), np.uint8)
    ...
    while True:
        try:
            ...
            if debug_flag == True:
                ...
                canvas[FRAME_HEIGHT:FRAME_HEIGHT*2,0:FRAME_WIDTH]=frame3
                canvas[FRAME_HEIGHT:FRAME_HEIGHT*2,FRAME_WIDTH:FRAME_WIDTH*2] = frame4
            ...
            if v[1] == 1:  #v[1] == f2q
                calc1 = CCRF_Lookup(v[0],frame1,sharedK.value)
                #calc_temp = ldr_tonemap_rgb_image_cv2(calc1/255,power ,radius)
                #calc_temp = calc_temp*255
                frame2 = v[0]
            elif v[1] ==2: #v[1] == f4q
                if CCRF_mode_flag == True:
                    calc2 = CCRF_Lookup(v[0],frame2,sharedK.value) #frame3 + frame2 --> calc2
                    calc4 = CCRF_Lookup(calc2,calc1,sharedK.value) #calc2 + calc1 --> calc4
                frame3 = v[0]
            elif v[1] ==3: #v[1] == f8q
                calc3 = CCRF_Lookup(v[0],frame3,sharedK.value) #frame4 + frame3 --> calc3
                if CCRF_mode_flag == True:
                    calc5 = CCRF_Lookup(calc3,calc2,sharedK.value) #calc3 + calc2 --> calc5
                    result_HDR = CCRF_Lookup(calc5,calc4,sharedK.value) #calc5 + calc4 --> result_HDR
                else:
                    result_HDR = CCRF_Lookup(calc3,calc1,sharedK.value)
                #print("Time taken to spit one HDR frame = ",time()-init_time)
                #init_time = time()
                frame4 = v[0]
                HDR_FRAME = ldr_tonemap_rgb_image_cv2(result_HDR/255,power ,radius)
                ...
                if userInput & 0xff == ord('w'):
                    ...
                    if CCRF_mode_flag == False:
                        if sharedK.value == 4:
                            #print("Doing 3 calculation mode, cannot go over k=4")
                            sharedK.value *= 2
