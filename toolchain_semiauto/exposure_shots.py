...
if os.path.isfile(flag_file) is True:
    raise ValueError("New photo need to be processed. Delete the flag file to bypass. Abort")

exposures=[base*k**i for i in range(20)]
set_id=str(uuid4())
IMG_FOLDER=os.path.join(data_dir,set_id)#"photo/{}".format(set_id)

maker_file=os.path.join(data_dir,marker_filename)
...
def main():
    ...
    # Release instance
    system.ReleaseInstance()
    with open(maker_file,'w') as fout:
        fout.write(set_id)
...
if __name__ == "__main__":
    ...
    main()
    s=input("Run comparagram now?(y/n) >")
    if s in ('y','Y','yes','Yes'):
        os.system("python comparagram_RGB_v1.py")
